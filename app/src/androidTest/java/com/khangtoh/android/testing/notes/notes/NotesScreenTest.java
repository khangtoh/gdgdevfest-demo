...
package com.khangtoh.android.testing.notes.notes;
...
public class NotesScreenTest {
    ...
    public void addNoteToNotesList() throws Exception {
        ...
        String noteDescription = "A description for the test note.";
        ...
        onView(withId(R.id.add_note_description))
                .perform(typeText(noteDescription),closeSoftKeyboard());

        // Click() on Add note button for Done
        onView(withId(R.id.fab_add_notes)).perform(click());

        // Verfiy note is now part of the list
        onView(withId(R.id.notes_list)).perform(
                scrollTo(hasDescendant(withText(noteDescription)))
        );

        onView(withItemText(noteDescription)).check(matches(isDisplayed()));
